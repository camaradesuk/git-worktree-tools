...
**For `show` subcommand:**
Import and call `formatConfigDisplay()` + `loadMergedConfig()` from `../../lib/global-config.js` (these are already imported in wtconfig.ts). If `--json` passed, wrap in `createSuccessResult`.
...
Import `loadMergedConfig`, `formatConfigDisplay` from `../../lib/global-config.js`.
...
Import `getConfigValue` from `../../lib/global-config.js`.
...
Import `setConfigValue` from `../../lib/global-config.js`.
...
Note: `getConfigPath` is already imported in the file. For the editor, use `getEditorCommand` from `../../lib/global-config.js` if available, or fall back to `EDITOR` env var.
...
- Mock `../../lib/global-config.js` for the new direct calls
