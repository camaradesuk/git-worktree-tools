...
must_haves:
  truths:
    ...
    - 'Running `lswt --verbose` produces the same logger output as `wt --verbose list` since both call gatherWorktreeInfo with verbose:true'
    - 'Legacy CLI --help outputs include a deprecation epilog directing to the wt equivalent'
  artifacts:
...
**Step 4: Add deprecation epilog to legacy CLI --help text**

For each legacy CLI that uses yargs, add an `.epilog()` call to the yargs chain:

```typescript
.epilog('DEPRECATED: Use "wt <command>" instead. This command will be removed in a future version.')
```

For CLIs that use a custom help formatter or `parseArgs()`, add the deprecation note to the help output string.

This ensures that running `lswt --help`, `newpr --help`, etc. includes a visible deprecation marker in the help body itself (in addition to the stderr notice from `printDeprecationNotice`).
</action>
<verify>
...
import { formatConfigDisplay, loadMergedConfig, setConfigValue } from '../../lib/wtconfig/index.js';
```
