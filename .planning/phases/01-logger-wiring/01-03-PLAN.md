...
must_haves:
  ...
  key_links:
    - from: 'src/lib/logger.test.ts'
      ...
      via: 'import { logger, initializeLogger, parseLogLevel, LogLevel, setAuditContext }'
      pattern: 'initializeLogger|parseLogLevel|setAuditContext'
---
...
- Before each test, reset the logger module state. Since consola instance is module-level, you may need to:
  - Set `logger.level` back to default
  - Clear reporters with `logger.setReporters([])`
  - Reset the deprecation-warned flag (may need an exported `_resetForTesting()` function in logger.ts — add one if needed, prefixed with `_` to signal internal-only)
