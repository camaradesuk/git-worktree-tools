...
must_haves:
  ...
  artifacts:
    - path: 'src/lib/logger.ts'
      ...
      exports:
        ['logger', 'initializeLogger', 'setAuditContext', 'parseLogLevel', '_resetForTesting']
...
<task type="auto">
  <name>Task 1: Add getGlobalDataDir to constants.ts and make colors.ts mutable</name>
  <files>
    src/lib/constants.ts
    src/lib/colors.ts
  </files>
...
- In `logger.ts` (Task 2), import `LogLevel` from `constants.js` and re-export it (this keeps backward compatibility).
...
<task type="auto">
  <name>Task 2: Install consola and replace logger.ts with consola wrapper</name>
  <files>
    package.json
    src/lib/logger.ts
  </files>
...
2. **Completely rewrite** `src/lib/logger.ts` (do NOT extend the existing implementation — per user decision, replace it). The new module must:
   ...
   **Exports:**
   ...
   - `LogLevel` — re-export from `constants.js` (compatibility mapping `{ SILENT, ERROR, WARN, INFO, DEBUG, TRACE }` to consola numeric values)
   - `_resetForTesting()` — resets module-level state (deprecation-warned flag, auditContext, reporters) for test isolation between test cases
   ...
   **`_resetForTesting()` function:**
   - Resets the module-level `deprecationWarned` boolean flag to `false`
   - Resets `auditContext` to its initial empty state
   - Clears logger reporters with `logger.setReporters([])`
   - Resets logger level to default (3 / INFO)
   - This is for test isolation only (prefixed with `_` to signal internal-only)
   ...
   **Important notes:**
   ...
   - Import `getGlobalDataDir` from `./constants.js` for the audit log path (already created in Task 1).
   - Import `LogLevel` from `./constants.js` and re-export it (already updated in Task 1).
     </action>
     <verify>
   - `npm run build` succeeds (TypeScript compiles — `getGlobalDataDir` exists in constants.ts from Task 1)
   - `import { logger, initializeLogger, parseLogLevel, LogLevel, _resetForTesting } from './logger.js'` resolves
   - `parseLogLevel('debug')` returns `4`, `parseLogLevel('error')` returns `0`
     </verify>
     <done>
     `src/lib/logger.ts` is a complete consola-based replacement with AuditFileReporter, ConditionalStderrReporter, audit session tracking, DEBUG=newpr deprecation warning, `_resetForTesting()`, and all documented exports. The old Logger class, ChildLogger class, and hand-rolled singleton are gone.
     </done>
     </task>
