...
import { runPrInteractiveMode } from '../../lib/prs/interactive.js';
import {
  createErrorResult,
  formatJsonResult,
  ErrorCode,
...
async function runPrsCommand(options: PrsCommandOptions): Promise<void> {
  ...
  try {
    ...
    prs = fetchPrsWithWorktrees(
      {
        state: fetchState,
        author: options.author === '@me' ? undefined : options.author, // GitHub CLI handles @me differently
  ...
  // Set author filter (for @me)
  if (options.author === '@me') {
    // We'll need to fetch current user - for now skip this filter on client side
    // The GitHub API handles @me natively
  }
